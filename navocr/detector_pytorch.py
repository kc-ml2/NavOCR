...
class PyTorchDetector(BaseDetector):
    def __init__(self, config: PyTorchDetectorConfig):
        ...
        try:
            from torch_engine.infer_only.core import YAMLConfig
        except ImportError as exc:
            raise ImportError(
                'Failed to import torch_engine.infer_only.core. Set detector_engine_root in the YAML '
                'params to the directory that contains the "torch_engine/infer_only" package.'
            ) from exc
    ...
    def _resolve_engine_root(configured: str | None) -> str | None:
        if configured:
            ...
            if os.path.isdir(os.path.join(root, 'torch_engine', 'infer_only')):
                return root
            if os.path.isdir(os.path.join(root, 'infer_only', 'core')):
                return os.path.dirname(root)
            if os.path.isdir(os.path.join(root, 'core')) and os.path.isfile(os.path.join(root, '__init__.py')):
                return os.path.dirname(os.path.dirname(root))
            raise FileNotFoundError(
                'detector_engine_root does not contain the "torch_engine/infer_only" package: '
                f'{root}'
            )

        # Default: NavOCR repo root (parent of the navocr package directory),
        # which holds the vendored torch_engine/infer_only tree.
        navocr_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
        if os.path.isdir(os.path.join(navocr_root, 'torch_engine', 'infer_only')):
            return navocr_root
        return None
