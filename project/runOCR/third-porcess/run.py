        ...
        try:
            ...
            for i, single_word in enumerate(all_words):
                ...
                save_image_info_to_csv(csv_path, image_file, single_word,
                                       x1, y1, x2, y2, center_x, center_y, boxes[i], scores[i])
...
def compare(save_path, ocr_path, yolo_path):
    ...
    # 비교 결과를 저장할 디렉토리 설정
    COMPARE_OUTPUT_DIR = os.path.join("/home/sooyong/datasets/output-3/", "compare_results")
        ...
        for row in ocr_reader:
            ...
            if ocr_file_name in yolo_dict:
                for yolo_entry in yolo_dict[ocr_file_name]:
                    ...
                    if (yolo_box['x1'] < ocr_center_x < yolo_box['x2']) and (yolo_box['y1'] < ocr_center_y < yolo_box['y2']):
                        # CSV 저장
                        save_prominent_sign_csv(
                            save_path,
                            ocr_file_name,
                            ocr_result,
                            ocr_x1,  # x1
...
def main():
    ocr_first()
    compare("/home/sooyong/datasets/output-3/result.csv", "/home/sooyong/datasets/output-3/ocr_info.csv", "/home/sooyong/datasets/output-3/yolo_info.csv")
